...
class PostRepository extends RepositoriesAbstract implements PostInterface
{
    ...
    public function getByCategoryExNews($category_id, $paginate = 12, $limit = 0)
    {
        if (!is_array($category_id)) {
            $category_id = [$category_id];
        }
        $data = $this->model
            ...
            ->join('categories', 'post_category.category_id', '=', 'categories.id')
            // ->whereIn('post_category.category_id', $category_id)
            ->where('post_category.category_id', '!=', 30)
            ->select('posts.*')
            ->distinct()
            ->orderBy('posts.created_at', 'desc');
        if ($paginate != 0) {
    ...
    public function getByCategoryNews($category_id, $paginate = 12, $limit = 0)
    {
        if (!is_array($category_id)) {
            $category_id = [$category_id];
        }
